...
const MIDIS_DATA = {
  ...
  scenarios: [
    ...
    },
    {
      ...
      goalSummary: 'Выделить критически важный минимум знаний по предмету, составить поминутный таймлайн подготовки на 24 часа и сформировать спасительные опорные сигналы для сдачи зачета.',
    ...
    },
    {
      ...
      goalSummary: 'Составить аудит текущих навыков студента, матрицу пробелов (Skill Gap Matrix) для целевой роли и 90-дневную траекторию плавного перехода без потери успеваемости.',
...
2. Напиши сценарий профессионального диалога (10 реплик) для обсуждения рабочего проекта с англоязычным коллегой.`;
    ...
    case 'lecture-summary':
      prompt += `1. Выдели 5 главных смысловых тезисов из представленного материала без "воды".
...
4. Сформулируй 5 контрольных вопросов для самопроверки с краткими эталонными ответами.`;
      break;

    case 'quiz-self-test':
      prompt += `1. Сгенерируй интерактивный тест из 10 вопросов по указанной теме:
...
3. В конце блока приведи ключ с подробным разбором: почему верный ответ правильный, и в чем типичная ошибка остальных вариантов.`;
    ...
    case 'exam-rescue-sprint':
      prompt += `1. Примени принцип Парето (матрица 80/20): выдели 20% фундаментальных тем, знание которых гарантирует 80% успеха на сдаче зачета/экзамена.
2. Составь поминутный аварийный таймлайн подготовки на 24 часа (с учетом 7 часов сна и коротких перерывов).
...
4. Дай 3 психологических приема для быстрой концентрации и снятия паники перед сдачей.`;
      break;

    case 'freelance-pricing-quote':
      prompt += `1. Проведи декомпозицию задачи на конкретные производственные этапы (исследование, прототип, итерации дизайна/кода, тестирование, сдача).
2. Рассчитай экономику заказа:
   - Диапазон часов на каждый этап;
...
3. Напиши профессиональный текст коммерческого предложения (КП) клиенту с обоснованием ценности и фиксацией условий (предоплата 50%, максимум 2 раунда правок).`;
    ...
    case 'career-switch-entry':
      prompt += `1. Составь матрицу переноса навыков (Transferable Skills): какие знания из текущей специальности (${spec.name}) дают мне преимущество в новой сфере.
2. Сформируй карту дефицитов (Skill Gap): какие ключевые hard-скиллы и инструменты нужно освоить в первую очередь.
3. Разработай 90-дневный план перехода:
   - Месяц 1: базовые пет-проекты и софт;
   - Месяц 2: полноценный сильный проект в портфолио;
   - Месяц 3: упаковка резюме и выход на первые отклики / стажировки.
4. Порекомендуй 3 профильных проекта, которые можно реализовать в проектных мастерских МИДиС для портфолио.`;
      break;

    case 'networking-personal-brand':
      prompt += `1. Сформулируй мою концепцию экспертного позиционирования (УТП студента: специальность + фокусная страсть).
2. Составь контент-план на первый месяц (12 постов для Telegram / соцсетей):
...
3. Напиши 3 шаблона первого сообщения (Cold Outreach) в Telegram:
   ...
   - Знакомство с коллегой-студентом для совместного проекта в FDE Lab.`;
    ...
    case 'user-flow-cjm':
      prompt += `1. Опиши персону пользователя (архетип, контекст использования, мотивы, ключевые ограничения).
...
3. Сформулируй 3 ключевые метрики успешности спроектированного пути (UX/бизнес-метрики).`;
      break;

    case 'ux-neuro-copywriting':
      prompt += `1. Разработай базовые принципы Tone of Voice (голос бренда): 3 прилагательных-ориентира и примеры «как надо / как не надо».
2. Напиши микротексты для ключевых экранов интерфейса:
   ...
   - 3 мотивирующих пуш-уведомления с заботливым тоном.`;
    ...
    case 'brand-identity-pack':
      prompt += `1. Сформулируй текстовую бренд-платформу:
...
2. Составь техническое задание на визуальную айдентику:
   ...
   - 3 идеи креативного брендированного мерча, который захочется носить каждый день.`;
    ...
    case 'hard-debug-solver':
      prompt += `1. Локализуй первопричину (Root Cause): почему именно возникает описанная ошибка/аномалия и в чем механика сбоя.
...
4. Дай 2 превентивных правила / линтинг-правила для предотвращения подобных ошибок в будущем.`;
      break;
